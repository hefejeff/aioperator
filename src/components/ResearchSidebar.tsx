...
const ResearchSidebar: React.FC<ResearchSidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Side Drawer */}
      <aside
        className={`fixed top-16 right-0 bottom-0 w-80 bg-wm-white border-l border-wm-neutral shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
          ...
          <div className="flex items-center justify-between mb-4 pb-4 border-b border-wm-neutral">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  onClick={onClose}
                  className="p-2 hover:bg-wm-neutral/40 rounded-lg transition-colors lg:hidden ml-auto text-wm-blue"
                  title="Close drawer"
                >
                  <Icons.X />
                </button>
              ...
              <div>
                <h3 className="text-sm font-bold text-wm-blue/60 mb-2">{t('research.findNew')}</h3>
                <div className="flex gap-2">
                  <button
                    onClick={onFindOpportunities}
                    disabled={isLoadingOpportunities}
                    className="flex-1 px-3 py-1.5 bg-wm-accent text-wm-white text-sm font-bold rounded-lg hover:bg-wm-accent/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-wm-pink text-wm-white text-sm font-bold rounded-lg hover:bg-wm-pink/90 transition-colors flex items-center justify-center gap-1"
                  >
                    <Icons.Plus className="w-4 h-4" />
                    {t('research.suggestOpportunity')}
                  </button>
                </div>
                <div className="mt-4 border-b border-wm-neutral" />
              </div>
            </div>
          </div>

          {/* Selected Scenarios Section */}
          <div className="mb-4 pb-4 border-b border-wm-neutral">
              ...
              <div className="space-y-2">
                {relatedScenarios
                  ...
                  .map(scenario => (
                    <div 
                      key={scenario.id}
                      className="flex items-center justify-between p-2 rounded bg-wm-neutral/20 border border-wm-neutral"
                    >
                      <button
            ...
            {relatedScenarios.length === 0 ? (
              ...
              <div className="space-y-3 overflow-y-auto h-full pr-2">
                {relatedScenarios.map((scenario) => (
                  <div
                    key={scenario.id}
                    className="rounded-lg bg-wm-neutral/20 border border-wm-neutral"
                  >
                    {/* Scenario Header */}
                    <div
                      ...
                      className="p-4 cursor-pointer hover:bg-wm-neutral/40 transition-colors group"
                    >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <div
                              ...
                              className="shrink-0 flex items-center justify-center w-5 h-5 rounded border cursor-pointer transition-colors hover:border-wm-accent hover:bg-wm-neutral/40 border-wm-neutral"
                            >
                              {selectedScenarios.includes(scenario.id) ? (
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-wm-accent" viewBox="0 0 20 20" fill="currentColor">
                                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                </svg>
                              ) : (
                                <div className="w-3 h-3 rounded transition-colors group-hover:bg-wm-accent/20" />
                              )}
                            </div>
                            <h3 className="text-sm font-bold text-wm-blue group-hover:text-wm-accent transition-colors">
                              {scenario.title}
                            </h3>
                          </div>
                        </div>
