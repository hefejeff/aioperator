...
const HistoryView: React.FC<{ user: firebase.User }> = ({ user }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      <h1 className="text-3xl font-bold text-center mb-8">Your Evaluation History</h1>
      {isLoading ? (
        ...
        <div className="text-center bg-slate-800/50 p-8 rounded-xl border border-slate-700">
            ...
            <h2 className="mt-4 text-xl font-bold">No History Found</h2>
            <p className="text-slate-400 mt-2">Complete some training or evaluation scenarios to see your progress here.</p>
        </div>
      ) : (
                  ...
                  <div key={item.id} className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-sky-500 transition-colors">
                    <div className="flex justify-between items-start gap-4">
                        <div className="flex-grow">
                            ...
                            <p className="font-semibold text-white mt-3">Feedback:</p>
                            <p className="text-slate-300 mt-1 whitespace-pre-wrap text-sm">{item.feedback}</p>
                            {item.workflowExplanation && (
                                <>
                                    <p className="font-semibold text-white mt-3">Your Workflow Explanation:</p>
                    ...
                    {item.imageUrl && (
                        <details className="mt-4">
                            <summary className="cursor-pointer text-sm text-sky-400 hover:underline">View Submitted Diagram</summary>
